...
public class ColonyBug : MonoBehaviour
{
    ...
    private EnemyBaseClass FindTarget()
    {
        List<EnemyBaseClass> allEnemies = EnemyManager.i.enemyList;
        List<EnemyBaseClass> allValidEnemies = new List<EnemyBaseClass>();

        foreach(EnemyBaseClass enemy in allEnemies)
        {
            if (enemy.gameObject.activeSelf)
                allValidEnemies.Add(enemy);
        }

        return allValidEnemies[Random.Range(0, allValidEnemies.Count)];
    }
    ...
    public void SetColonyParent(Colony parent)
    {
        this.parent = parent;
    }
}
